...
% david.katzin1@gmail.com
    ...
    %% Lamp parameters
    % LED
    setParam(gl, 'thetaLampMax', 200/3);               % Maximum intensity of lamps																	[W m^{-2}]                              Pg. 48 [1] (measured capacity divided by 144 m2)
    setParam(gl, 'heatCorrection', 0);   			% correction for temperature setpoint when lamps are on 										[C]   									0
    setParam(gl, 'etaLampPar', 3/5.41);               % fraction of lamp input converted to PAR 														[-]                                     [3] gives 0.33 with a lamp of 1.7 umol/J. This lamp is 1.6 umol/J [2], i.e. 6% less efficient as the one in [3]
    setParam(gl, 'etaLampNir', 0.02);               % fraction of lamp input converted to NIR 														[-]                                     [3]
    setParam(gl, 'tauLampPar', 0.98);               % transmissivity of lamp layer to PAR 															[-]                                     Pg. 22 [1] (light loss due to armatures)
    setParam(gl, 'rhoLampPar', 0);                  % reflectivity of lamp layer to PAR 															[-]                                     0
    setParam(gl, 'tauLampNir', 0.98);               % transmissivity of lamp layer to NIR 															[-]                                     Pg. 22 [1] (light loss due to armatures)
    setParam(gl, 'rhoLampNir', 0);                  % reflectivity of lamp layer to NIR 															[-]                                     0
    setParam(gl, 'tauLampFir', 0.98);               % transmissivity of lamp layer to FIR 															[-]                                     Pg. 22 [1] (light loss due to armatures)
    setParam(gl, 'aLamp', 0.02);                    % lamp area 																					[m^{2}{lamp} m^{-2}{floor}]             Uitstralingsmonitor pg. 28 gives 0.02 for 100 W/m2 Son-T, later (pg. 35) gives 1% for each 100umol, and claims similar values for HPS and modern LEDs
    ...
    setParam(gl, 'etaLampCool', 0);                 % fraction of lamp input removed by cooling                                                     [-]                                     Estimate based on pg. 50 [1]
    setParam(gl, 'zetaLampPar', 5.41);            % J to umol conversion of PAR output of lamp                                                    [umol{PAR} J^{-1}]                      Efficacy of 1.6 umol/J [2], divided by 0.31 fraction to PAR (see above)

...
% david.katzin1@gmail.com
    ...
    %% Lamp parameters
    % HPS
    setParam(gl, 'thetaLampMax', 200/1.8);                  % Maximum intensity of lamps																	[W m^{-2}]                              Pg. 48 [1] (measured capacity divided by 144 m2)
    setParam(gl, 'heatCorrection', 0);   			% correction for temperature setpoint when lamps are on 										[C]   									0
    setParam(gl, 'etaLampPar', 1.8/4.9);               % fraction of lamp input converted to PAR 														[-]                                     [3] gives 0.34 with a lamp of 1.7 umol/J. This lamp is 1.8 umol/J [2], i.e. 6% more efficient as the one in [3]
    ...
    setParam(gl, 'aLamp', 0.02);                    % lamp area 																					[m^{2}{lamp} m^{-2}{floor}]             Uitstralingsmonitor pg. 28 gives 0.02 for 100 W/m2 Son-T, later (pg. 35) gives 1% for each 100umol 
    ...
    setParam(gl, 'zetaLampPar', 4.9);           	% J to umol conversion of PAR output of lamp                                                    [umol{PAR} J^{-1}]                 		Efficacy of 1.8 umol/J [2], divided by 0.36 fraction to PAR (see above)      

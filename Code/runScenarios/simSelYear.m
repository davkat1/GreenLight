function gl = simSelYear(simType, seasonLength, firstDay, fileLabel, mature, paramName, paramVal, intensity)
...
% firstDay = 300; % Beginning of season (days since January 1)
    ...
    [weather, startTime] = loadSelYearHiRes(firstDay, seasonLength); % load inputs
    weather(:,8) = soilTempNl(firstDay*86400+weather(:,1)); % add soil temperature
    
    weather = [weather dayLightSum(weather)']; % add daily light sum integral

    gl = createGreenLightModel(weather, startTime);
...
%     gl.u.thScr = min(gl.a.thScrCold, gl.a.thScrHeat);
    ...
    if exist('paramName', 'var') && ~isempty(paramName)
        for k=1:length(paramName)
            setParam(gl, paramName(k), paramVal(k));
        end
    end
    ...
    if saveFile
        save(['selYear_' fileLabel '_' simType '_' startTime '_' num2str(seasonLength) 'days.mat']);
    end

function gl = simEnergyPlusLoadHpsTransp(location, simType, seasonLength, firstDay, fileLabel, mature, intensity)
...
% firstDay = 300; % Beginning of season (days since January 1)
    ...
    %% turn transpiration into an input and use the one from the HPS lamp
    load('M:\PhD\Models\git\gitwur\GreenLight\mvCanAirHpsNoLampLimit.mat', 'mvCanAirHpsNoLampLimit');
    addInput(gl, 'mvCanAir', mvCanAirHpsNoLampLimit.val);
...
%         setParam(gl, 'etaLampPar', 0.67);
    ...
    %% No limit on lamps
    setParam(gl, 'lampsOffSun', 1e6);
    setParam(gl, 'lampRadSumLimit', 1e6);

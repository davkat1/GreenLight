...
Example usage:
    ...
    >>> convert_energy_plus(energy_plus_csv_path,output_file_path,format=katzin2021)
...
def convert_energy_plus(
    ...
    # Open the EnergyPlus file and store the data in a dataframe
    df_ep = pd.read_csv(energy_plus_csv_path, header=18, encoding_errors="ignore")
...
def _get_hourly_deep_soil_temperature(energy_plus_csv_path: str) -> np.array:
    ...
    soil_2m_temp = pd.read_csv(energy_plus_csv_path, skiprows=8, nrows=1, encoding_errors="ignore", header=None)
